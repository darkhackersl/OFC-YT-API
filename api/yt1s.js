...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: mp3Html } = await axios.get(`https://api.yt-download.org/api/button/mp3/${videoId}`);
    ...
    const { data: mp4Html } = await axios.get(`https://api.yt-download.org/api/button/videos/${videoId}`);
